...
contract OxGammaDAO is Ownable {
    struct Proposal {
        ...
        mapping(uint256 => bool) voters; // 0xGammaNFT tokenId => bool (true if nft used to cast a vote, false if not).
    }
    ...
    constructor(address _nftMarketplace, address _OxGammaNFT) payable {
        //payable => cosntructor can receive ether when deployed

        nftMarketplace = IFakeNFTMarketplace(_nftMarketplace);
        OxGammaNFT = IOxGammaNFT(_OxGammaNFT);
    }
    
    modifier nftHolderOnly() {
        // can be called by someone who owns atlest one OxGammaNFT
        require(OxGammaNFT.balanceOf(msg.sender) > 0, "NOT_A_DAO_MEMBER");
        _;
    }
    ...
    function voteOnProposal(uint256 proposalIndex, Vote vote) external nftHolderOnly activeProposalOnly(proposalIndex){
        ...
        uint256 voterNFTBalance = OxGammaNFT.balanceOf(msg.sender);
        uint256 numVotes = 0;

        // calculating number of NFTs owned by voter which haven't already been used for voting on this proposal
        for (uint256 i = 0; i < voterNFTBalance; i++) {
            uint256 tokenId = OxGammaNFT.tokenOfOwnerByIndex(msg.sender, i);
