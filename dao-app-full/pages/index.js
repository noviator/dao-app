...
export default function Home() {
  ...
  const [nftBalance, setNftBalance] = useState(0); // user's 0xGammaNFT NFT balance
  ...
  const getUserNFTBalance = async () => {
    // get the user's 0xGammaNFT balance
    try {
      const signer = await getProviderOrSigner(true);
      const nftContract = get0xGammaNFTContractInstance(signer);
  ...
  function renderCreateProposalTab() {
    ...
    if (loading) {
      ...
      return (
        <div className={styles.description}>
          You do not own any 0xGammaNFT NFTs. 
          <br />
  ...
  return (
    <div>
      <Head>
        <title>0xGamma DAO</title>
        <meta name="description" content="0xGamma DAO" />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Welcome to 0xGamma!</h1>
          <div className={styles.description}>Welcome to the DAO!</div>
          <div className={styles.description}>
            Your 0xGammaNFT Balance: {nftBalance}
            <br />
